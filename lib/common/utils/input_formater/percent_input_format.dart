...
class PercentTextFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    String decimalSymbol = (formatter ?? _formatter).symbols.DECIMAL_SEP;
    if (allowFraction &&
        newValue.text.contains(decimalSymbol) &&
        (newValue.text.split(decimalSymbol).last.length > 1 || newValue.text.split(decimalSymbol).length >= 3)) {
      return oldValue;
    }
