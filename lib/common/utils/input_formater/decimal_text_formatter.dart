...
class DecimalTextFormatter extends TextInputFormatter {
  static final NumberFormat _defaultFormatter =
      NumberFormat.decimalPattern(Platform.localeName);
  static const int _maximumFractionDigits = 9;
  ...
  DecimalTextFormatter({this.formatter, this.allowFraction = false})
      : _decimalSeparator =
            (formatter ?? _defaultFormatter).symbols.DECIMAL_SEP,
        _decimalRegex = RegExp(allowFraction
            ? '[0-9]+([${(formatter ?? _defaultFormatter).symbols.DECIMAL_SEP}])?'
            : r'\d+'),
  ...
  String _formatPattern(String? digits) {
    ...
    if (allowFraction) {
      if (digits.contains(_decimalSeparator)) {
        ...
        for (var i = decimalPart.length - 1;
            i >= 0 && decimalPart[i] == "0";
            i--) {
    ...
    (formatter ?? _defaultFormatter).maximumFractionDigits =
        _maximumFractionDigits;
    final result = (formatter ?? _defaultFormatter).format(number);
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (allowFraction &&
        newValue.text
            .contains((formatter ?? _defaultFormatter).symbols.DECIMAL_SEP) &&
        (newValue.text
                    .split((formatter ?? _defaultFormatter).symbols.DECIMAL_SEP)
                    .last
                    .length >
                _maximumFractionDigits ||
            newValue.text
