...
class RegexPattern {
  static RegExp numericText = RegExp(r'[0-9]');
  static RegExp notNumericText = RegExp('[^0-9]');
  ...
  static const Pattern phone = r'^(?:[+0][1-9])?[0-9]{10,12}$';
  ...
  static const Pattern leastOneLetter  =
      r'^(?=.*[A-Za-z])';
  
  /// Matches least one capital letter  
  static const Pattern leastOneCapitalLetter  =
      r'^(?=.*[A-Z])';
  
  /// Matches least one number  
  static const Pattern leastOneNumber  =
      r'^(?=.*[0-9])';
  /// Matches least one special  
  static const Pattern leastOneSpecialLetter  =
      r'^(?=.*[\W_])';
  
  
  static const Pattern passwordCustom  =
            r'^(?=(.*[0-9]))(?=.*[!@#$%^&*])(?=.*[a-z])(?=(.*[A-Z]))(?=(.*))\S{8,255}$';
}
